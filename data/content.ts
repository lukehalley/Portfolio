...
export const journey = [
  `At **[redacted]** (${formatDateRange([redacted]_START)}), I led the multi-cloud transformation—migrating from **AWS CDK to Terraform** to enable operations across **AWS and Azure**. Built a self-hosted Terraform solution replacing Terraform Cloud, then deployed it across **95 accounts** via Azure DevOps Pipelines.`,
    ...
    highlights: [
      {
        ...
        description: "Migrated [redacted] **from AWS CDK to Terraform** to enable **multi-cloud strategy** across AWS and Azure. Designed and built **self-hosted Terraform solution** replacing Terraform Cloud, then deployed infrastructure across **95 AWS accounts** using Azure DevOps Pipelines with ServiceNow integration for change management.",
